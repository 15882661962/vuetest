<template>
  <div class="index">
    ...
    <recharge-count :costCount=costCount :rechargeCount=rechargeCount :timeLabel=timeLabel></recharge-count>
    <recharge-player :players=players v-if="players.length"></recharge-player>
      ...
      statistics(data).then(
        res => {
          for (var i = 0; i < res.Data.length; i++) {
            var player = res.Data[i]
            let playerobj = {
              info: player.NickName + '（ID:' + player.UserId + '）[' + player.TypeId + ']',
              imgurl: 'http://www.7uao.com/upload/headimg/' + player.UserId + '.jpg',
              GamerCount: player.GamerCount,
              Total: player.Total,
              SpreadName: player.SpreadName,
              Gold: player.Gold,
              Cost: player.Cost
            }
            this.costCount += player.Cost
            this.rechargeCount += player.Total
